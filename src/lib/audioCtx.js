...
export function Envelope(adsr, audioParam) {
  ...
  const hold = () => {
    ...
    extenteded.push(ctx.currentTime);
    audioParam.cancelScheduledValues(0);
  ...
  return {
    ...
    triggerEnvelope: ({ attackStart, releaseStart, extended }) => {
      ...
      var sustainedTime = extended.length ? extended[extended.length - 1] : 0;

      audioParam.setValueCurveAtTime([0, peak], ctx.currentTime, attacked);
      audioParam.setValueCurveAtTime(
        [peak, sustain * peak],
        ctx.currentTime + sustainedTime,
        sustainedTime
      );
      audioParam.setTargetAtTime(0.0000001, ctx.currentTime + attack + sustainedTime, release);
    },
    cloneShape: () => [attackStart, releaseStart, extenteded],
  };
}
