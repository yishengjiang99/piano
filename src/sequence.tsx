...
const Sequence = ({
  ...
  const [fftc, postFftc] = useChannel<{ dataArray: Float32Array; binCount: number }>("fftc");
  const [zoomX, setZoomX] = useState(1);
  const canvasRef = useRef<HTMLCanvasElement>();
  const canvasHudRef = useRef<HTMLCanvasElement>();
  const canvasFFTRef = useRef<HTMLCanvasElement>();
  ...
  useEffect(() => {
    ...
    if (!canvasRef.current || !canvasFFTRef.current) {
      throw new Error("canvasRef or canvasFFTRef not foudn");
    }
    barCursor > 0 && _drawAxis();
  }, [canvasRef, canvasFFTRef, barCursor, canvasWidth, canvasHeight, rows, cols]);

  useEffect(() => {
    if (canvasFFTRef.current) {
  ...
  useEffect(() => {
    if (fftc.lastMessage) {
      const { dataArray, binCount } = fftc.lastMessage;
      const canvasFFTCtx = canvasFFTRef.current.getContext("2d");
